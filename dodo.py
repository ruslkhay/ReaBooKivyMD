...
def task_test():
    ...
    yield {"actions": ["coverage report -m"], "verbosity": 2, "name": "report"}


def task_peps():
    """Check if all .py files are satisfies PEP below."""
    ignore_dir = (
        ".buildozer,.git,.venv,__pycache__,build,dist," + "CardStack,Swiper,source"
    )
    yield {"actions": ["flake8 --extend-exclude={}".format(ignore_dir)], "name": "pep8"}
    yield {
        "actions": [
            "pydocstyle --match-dir='^(?!.*(\\{})).*$'".format(
...
def task_po():
    ...
    return {
        "actions": [
            "pybabel update --ignore-pot-creation-date -D reaboo "
...
def task_buildozer():
    """Generate apk file."""
    return {"actions": ["buildozer android debug"]}
